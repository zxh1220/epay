...
{
	static public $info = [
		'name'        => 'lzyzf', //支付插件英文名称，需和目录名称一致，不能有重复
		'showname'    => '其他易支付', //支付插件显示名称
		'author'      => '浪子', //支付插件作者
		'link'        => 'http://www.79k.top/', //支付插件作者链接
	...
	static public function submit(){
		...
		require(PAY_ROOT."inc/lzyzf.config.php");
		require(PAY_ROOT."inc/LpayCore.class.php");
		$parameter = array(
			"pid" => trim($lzyzf_config['pid']),
			"type" => $order['typename'],
		...
		//建立请求
		$lzyzf = new lpayCore($lzyzf_config);
		if(is_https() && substr($lzyzf_config['apiurl'],0,7)=='http://'){
			$jump_url = $lzyzf->getPayLink($parameter);
			return ['type'=>'jump','url'=>$jump_url];
		}else{
			$html_text = $lzyzf->pagePay($parameter, '正在跳转');
	...
	static private function pay_mapi($type){
		...
		return \lib\Payment::lockPayData(TRADE_NO, function() use($lzyzf, $parameter) {
			$result = $lzyzf->apiPay($parameter);
	...
	static public function notify(){
		...
		//计算得出通知验证结果
		$epayNotify = new LpayCore($lzyzf_config);
	...
	static public function refund($order){
		...
		$lzyzf = new LpayCore($lzyzf_config);
		$result = $lzyzf->refund($order['api_trade_no'], $order['refundmoney']);

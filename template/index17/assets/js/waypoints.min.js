!(function() {
	function c(d) {
		...
		}(this.key = "waypoint-" + a), (this.options = c.Adapter.extend({}, c.defaults, d)), (this.element = this
			.options.element), (this.adapter = new c.Adapter(this.element)), (this.callback = d.handler), (this
			.axis = this.options.horizontal ? "horizontal" : "vertical"), (this.enabled = this.options.enabled), (
			this.triggerPoint = null), (this.group = c.Group.findOrCreate({
	...
	function a(e) {
		(this.element = e), (this.Adapter = c.Adapter), (this.adapter = new this.Adapter(e)), (this.key =
			"waypoint-context-" + b), (this.didScroll = !1), (this.didResize = !1), (this.oldScroll = {
		...
		for (var D in F) {
			...
			for (var j in this.waypoints[D]) {
				var A, x, C, G, k, m = this.waypoints[D][j],
					...
					I = 0,
					v = null == H;
				m.element !== m.element.window && (I = m.adapter.offset()[E.offsetProp]), "function" ==
					typeof q ? (q = q.apply(m)) : "string" == typeof q && ((q = parseFloat(q)), m.options.offset
						.indexOf("%") > -1 && (q = Math.ceil((E.contextDimension * q) / 100))), (A = E
						.contextScroll - E.contextOffset), (m.triggerPoint = Math.floor(I + A - q)), (x = H < E
						.oldScroll), (C = m.triggerPoint >= E.oldScroll), (G = x && C), (k = !x && !C), !v &&
					G ? (m.queueTrigger(E.backward), (B[m.group.id] = m.group)) : !v && k ? (m.queueTrigger(E
						.forward), (B[m.group.id] = m.group)) : v && E.oldScroll >= m.triggerPoint && (m
						.queueTrigger(E.forward), (B[m.group.id] = m.group))
			}
		}
	...
	function b(c) {
		return function() {
			var d = [],
				e = arguments[0];
			return (c.isFunction(arguments[0]) && ((e = c.extend({}, arguments[1])), (e.handler = arguments[
				0])), this.each(function() {
					var f = c.extend({}, e, {
						element: this
					});
					"string" == typeof f.context && (f.context = c(this).closest(f.context)[0]), d.push(
						new a(f))
				}), d)
		}
	}

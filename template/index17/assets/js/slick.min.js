(function(a) {
	...
	b.prototype.buildDots = function() {
		...
		if (c.options.dots === true && c.slideCount > c.options.slidesToShow) {
			...
			c.$dots.find("li").first().addClass("slick-active")
		}
	};
	b.prototype.buildOut = function() {
		...
		c.$slideTrack = c.slideCount === 0 ? a('<div class="slick-track"/>').appendTo(c.$slider) : c.$slides
	...
	b.prototype.cleanUpEvents = function() {
		...
		if (c.options.dots && c.$dots !== null) {
			a("li", c.$dots).off("click.slick", c.changeSlide).off("mouseenter.slick", a.proxy(c.interrupt, c,
				true)).off("mouseleave.slick", a.proxy(c.interrupt, c, false));
	...
	b.prototype.destroy = function(d) {
		...
		if (c.$prevArrow && c.$prevArrow.length) {
			c.$prevArrow.removeClass("slick-disabled slick-arrow slick-hidden").removeAttr(
		...
		if (c.$slides) {
			c.$slides.removeClass("slick-slide slick-active slick-center slick-visible slick-current")
		...
		if (c.options.infinite === true) {
			if (c.slideCount <= c.options.slidesToShow) {
				...
				while (d < c.slideCount) {
					...
					e += c.options.slidesToScroll <= c.options.slidesToShow ? c.options.slidesToScroll : c
	...
	b.prototype.getLeft = function(e) {
		var c = this,
			f, h, i = 0,
			g, d;
		c.slideOffset = 0;
		h = c.$slides.first().outerHeight(true);
		if (c.options.infinite === true) {
			if (c.slideCount > c.options.slidesToShow) {
				c.slideOffset = c.slideWidth * c.options.slidesToShow * -1;
				d = -1;
				if (c.options.vertical === true && c.options.centerMode === true) {
					if (c.options.slidesToShow === 2) {
						d = -1.5
					} else {
						if (c.options.slidesToShow === 1) {
							d = -2
						}
					}
				}
				i = h * c.options.slidesToShow * d
			}
			if (c.slideCount % c.options.slidesToScroll !== 0) {
				if (e + c.options.slidesToScroll > c.slideCount && c.slideCount > c.options.slidesToShow) {
					if (e > c.slideCount) {
						c.slideOffset = (c.options.slidesToShow - (e - c.slideCount)) * c.slideWidth * -1;
						i = (c.options.slidesToShow - (e - c.slideCount)) * h * -1
					} else {
						c.slideOffset = c.slideCount % c.options.slidesToScroll * c.slideWidth * -1;
						i = c.slideCount % c.options.slidesToScroll * h * -1
					}
				}
			}
		} else {
			if (e + c.options.slidesToShow > c.slideCount) {
				c.slideOffset = (e + c.options.slidesToShow - c.slideCount) * c.slideWidth;
				i = (e + c.options.slidesToShow - c.slideCount) * h
			}
		}
		if (c.slideCount <= c.options.slidesToShow) {
			c.slideOffset = 0;
		...
		if (c.options.centerMode === true && c.slideCount <= c.options.slidesToShow) {
			c.slideOffset = c.slideWidth * Math.floor(c.options.slidesToShow) / 2 - c.slideWidth * c
				.slideCount / 2
		} else {
		...
		if (c.options.vertical === false) {
			f = e * c.slideWidth * -1 + c.slideOffset
		} else {
			f = e * h * -1 + i
		}
		if (c.options.variableWidth === true) {
			if (c.slideCount <= c.options.slidesToShow || c.options.infinite === false) {
				g = c.$slideTrack.children(".slick-slide").eq(e)
			} else {
				g = c.$slideTrack.children(".slick-slide").eq(e + c.options.slidesToShow)
			}
			if (c.options.rtl === true) {
				if (g[0]) {
					f = (c.$slideTrack.width() - g[0].offsetLeft - g.width()) * -1
				} else {
					f = 0
				}
			} else {
				f = g[0] ? g[0].offsetLeft * -1 : 0
			}
			if (c.options.centerMode === true) {
				if (c.slideCount <= c.options.slidesToShow || c.options.infinite === false) {
					g = c.$slideTrack.children(".slick-slide").eq(e)
				} else {
					g = c.$slideTrack.children(".slick-slide").eq(e + c.options.slidesToShow + 1)
				}
				if (c.options.rtl === true) {
					if (g[0]) {
						f = (c.$slideTrack.width() - g[0].offsetLeft - g.width()) * -1
	...
	b.prototype.getNavigableIndexes = function() {
		...
		while (d < g) {
			f.push(d);
			d = e + c.options.slidesToScroll;
			e += c.options.slidesToScroll <= c.options.slidesToShow ? c.options.slidesToScroll : c.options
				.slidesToShow
		}
		return f
	};
	...
	b.prototype.getSlideCount = function() {
		var c = this,
			e, f, g, d;
		d = c.options.centerMode === true ? Math.floor(c.$list.width() / 2) : 0;
		g = c.swipeLeft * -1 + d;
		if (c.options.swipeToSlide === true) {
			c.$slideTrack.find(".slick-slide").each(function(h, i) {
				var k, j, l;
				k = a(i).outerWidth();
				j = i.offsetLeft;
				if (c.options.centerMode !== true) {
	...
	b.prototype.initADA = function() {
		var c = this,
			f = Math.ceil(c.slideCount / c.options.slidesToShow),
			g = c.getNavigableIndexes().filter(function(h) {
				return h >= 0 && h < c.slideCount
			});
		...
		if (c.$dots !== null) {
			c.$slides.not(c.$slideTrack.find(".slick-cloned")).each(function(j) {
				var k = g.indexOf(j);
				a(this).attr({
					role: "tabpanel",
					id: "slick-slide" + c.instanceUid + j,
					tabindex: -1
				});
				if (k !== -1) {
					var h = "slick-slide-control" + c.instanceUid + k;
					if (a("#" + h).length) {
						a(this).attr({
							"aria-describedby": h
						})
			...
			c.$dots.attr("role", "tablist").find("li").each(function(h) {
				var j = g[h];
				a(this).attr({
					role: "presentation"
				});
				a(this).find("button").first().attr({
					role: "tab",
					id: "slick-slide-control" + c.instanceUid + h,
					"aria-controls": "slick-slide" + c.instanceUid + j,
					"aria-label": h + 1 + " of " + f,
					"aria-selected": null,
					tabindex: "-1"
				})
		...
		for (var d = c.currentSlide, e = d + c.options.slidesToShow; d < e; d++) {
			if (c.options.focusOnChange) {
				c.$slides.eq(d).attr({
					tabindex: "0"
				})
			} else {
				c.$slides.eq(d).removeAttr("tabindex")
			}
		}
		c.activateADA()
	};
	...
	b.prototype.initDotEvents = function() {
		...
		if (c.options.dots === true && c.options.pauseOnDotsHover === true && c.slideCount > c.options
			.slidesToShow) {
			a("li", c.$dots).on("mouseenter.slick", a.proxy(c.interrupt, c, true)).on("mouseleave.slick", a
				.proxy(c.interrupt, c, false))
		}
	};
	...
	b.prototype.lazyLoad = function() {
		...
		function g(i) {
			a("img[data-lazy]", i).each(function() {
				var n = a(this),
					p = a(this).attr("data-lazy"),
					q = a(this).attr("data-srcset"),
					o = a(this).attr("data-sizes") || d.$slider.attr("data-sizes"),
				...
				r.onload = function() {
					n.animate({
						...
						n.attr("src", p).animate({
							opacity: 1
						}, 200, function() {
							n.removeAttr("data-lazy data-srcset data-sizes")
				...
				r.onerror = function() {
					n.removeAttr("data-lazy").removeClass("slick-loading").addClass(
						"slick-lazyload-error");
					d.$slider.trigger("lazyLoadError", [d, n, p])
				};
				r.src = p
			})
		}
		if (d.options.centerMode === true) {
			if (d.options.infinite === true) {
				m = d.currentSlide + (d.options.slidesToShow / 2 + 1);
				l = m + d.options.slidesToShow + 2
			} else {
				m = Math.max(0, d.currentSlide - (d.options.slidesToShow / 2 + 1));
				l = 2 + (d.options.slidesToShow / 2 + 1) + d.currentSlide
			}
		} else {
			m = d.options.infinite ? d.options.slidesToShow + d.currentSlide : d.currentSlide;
			l = Math.ceil(m + d.options.slidesToShow);
		...
		h = d.$slider.find(".slick-slide").slice(m, l);
		if (d.options.lazyLoad === "anticipated") {
			var k = m - 1,
				j = l,
				c = d.$slider.find(".slick-slide");
			for (var f = 0; f < d.options.slidesToScroll; f++) {
				if (k < 0) {
					k = d.slideCount - 1
				}
				h = h.add(c.eq(k));
		...
		g(h);
		if (d.slideCount <= d.options.slidesToShow) {
			e = d.$slider.find(".slick-slide");
			g(e)
		} else {
			if (d.currentSlide >= d.slideCount - d.options.slidesToShow) {
				e = d.$slider.find(".slick-cloned").slice(0, d.options.slidesToShow);
				g(e)
			} else {
				if (d.currentSlide === 0) {
					e = d.$slider.find(".slick-cloned").slice(d.options.slidesToShow * -1);
					g(e)
				}
			}
		}
	...
	b.prototype.postSlide = function(e) {
		var d = this;
		if (!d.unslicked) {
			d.$slider.trigger("afterChange", [d, e]);
			d.animating = false;
			if (d.slideCount > d.options.slidesToShow) {
				d.setPosition()
			}
			...
			if (d.options.accessibility === true) {
				d.initADA();
				if (d.options.focusOnChange) {
					var c = a(d.$slides.get(d.currentSlide));
	...
	b.prototype.progressiveLazyLoad = function(j) {
		j = j || 1;
		var d = this,
		...
		if (c.length) {
			e = c.first();
			g = e.attr("data-lazy");
			h = e.attr("data-srcset");
			f = e.attr("data-sizes") || d.$slider.attr("data-sizes");
			...
			i.onload = function() {
				...
				e.attr("src", g).removeAttr("data-lazy data-srcset data-sizes").removeClass(
				"slick-loading");
				if (d.options.adaptiveHeight === true) {
					d.setPosition()
				}
				d.$slider.trigger("lazyLoaded", [d, e, g]);
				d.progressiveLazyLoad()
			};
			i.onerror = function() {
				if (j < 3) {
					setTimeout(function() {
						d.progressiveLazyLoad(j + 1)
					}, 500)
				} else {
					e.removeAttr("data-lazy").removeClass("slick-loading").addClass("slick-lazyload-error");
					d.$slider.trigger("lazyLoadError", [d, e, g]);
					d.progressiveLazyLoad()
				}
			};
			i.src = g
	...
	b.prototype.setDimensions = function() {
		...
		if (c.options.vertical === false && c.options.variableWidth === false) {
			...
			if (c.options.variableWidth === true) {
				...
				c.$slideTrack.height(Math.ceil(c.$slides.first().outerHeight(true) * c.$slideTrack.children(
					".slick-slide").length))
			}
		}
	...
	b.prototype.setSlideClasses = function(g) {
		var c = this,
			e, d, h, i;
		d = c.$slider.find(".slick-slide").removeClass("slick-active slick-center slick-current").attr(
			"aria-hidden", "true");
		c.$slides.eq(g).addClass("slick-current");
		if (c.options.centerMode === true) {
			var f = c.options.slidesToShow % 2 === 0 ? 1 : 0;
			e = Math.floor(c.options.slidesToShow / 2);
			if (c.options.infinite === true) {
				if (g >= e && g <= c.slideCount - 1 - e) {
					c.$slides.slice(g - e + f, g + e + 1).addClass("slick-active").attr("aria-hidden", "false")
				} else {
					h = c.options.slidesToShow + g;
					d.slice(h - e + 1 + f, h + e + 2).addClass("slick-active").attr("aria-hidden", "false")
				}
				if (g === 0) {
			...
			c.$slides.eq(g).addClass("slick-center")
		} else {
			if (g >= 0 && g <= c.slideCount - c.options.slidesToShow) {
				c.$slides.slice(g, g + c.options.slidesToShow).addClass("slick-active").attr("aria-hidden",
					"false")
			} else {
				if (d.length <= c.options.slidesToShow) {
					d.addClass("slick-active").attr("aria-hidden", "false")
				} else {
					i = c.slideCount % c.options.slidesToShow;
					h = c.options.infinite === true ? c.options.slidesToShow + g : g;
					if (c.options.slidesToShow == c.options.slidesToScroll && c.slideCount - g < c.options
						.slidesToShow) {
						d.slice(h - (c.options.slidesToShow - i), h + i).addClass("slick-active").attr(
							"aria-hidden", "false")
					} else {
						d.slice(h, h + c.options.slidesToShow).addClass("slick-active").attr("aria-hidden",
							"false")
					}
				}
			}
		}
		if (c.options.lazyLoad === "ondemand" || c.options.lazyLoad === "anticipated") {
			c.lazyLoad()
		}
	};
		...
		if (c.options.infinite === true && c.options.fade === false) {
			...
			if (c.slideCount > c.options.slidesToShow) {
				...
				for (d = c.slideCount; d > c.slideCount - e; d -= 1) {
					f = d - 1;
					a(c.$slides[f]).clone(true).attr("id", "").attr("data-slick-index", f - c.slideCount)
						.prependTo(c.$slideTrack).addClass("slick-cloned")
				}
				for (d = 0; d < e + c.slideCount; d += 1) {
					f = d;
					a(c.$slides[f]).clone(true).attr("id", "").attr("data-slick-index", f + c.slideCount)
						.appendTo(c.$slideTrack).addClass("slick-cloned")
				}
	...
	b.prototype.slideHandler = function(f, j, e) {
		...
		if (c.options.infinite === false && c.options.centerMode === false && (f < 0 || f > c.getDotCount() * c
				.options.slidesToScroll)) {
			if (c.options.fade === false) {
				l = c.currentSlide;
				if (e !== true && c.slideCount > c.options.slidesToShow) {
			...
			if (c.options.infinite === false && c.options.centerMode === true && (f < 0 || f > c.slideCount - c
					.options.slidesToScroll)) {
				if (c.options.fade === false) {
					l = c.currentSlide;
					if (e !== true && c.slideCount > c.options.slidesToShow) {
	...
	b.prototype.swipeEnd = function(e) {
		...
		if (c.touchObject.swipeLength >= c.touchObject.minSwipe) {
			...
			switch (d) {
				case "left":
				case "down":
					f = c.options.swipeToSlide ? c.checkNavigable(c.currentSlide + c.getSlideCount()) : c
						.currentSlide + c.getSlideCount();
	...
	b.prototype.swipeHandler = function(d) {
		...
		c.touchObject.fingerCount = d.originalEvent && d.originalEvent.touches !== undefined ? d.originalEvent
			.touches.length : 1;
	...
	b.prototype.swipeMove = function(f) {
		...
		if (!c.dragging || c.scrolling || j && j.length !== 1) {
			return false
		}
		...
		c.touchObject.swipeLength = Math.round(Math.sqrt(Math.pow(c.touchObject.curX - c.touchObject.startX,
			2)));
		k = Math.round(Math.sqrt(Math.pow(c.touchObject.curY - c.touchObject.startY, 2)));
		if (!c.options.verticalSwiping && !c.swiping && k > 4) {
			c.scrolling = true;
			return false
		}
		if (c.options.verticalSwiping === true) {
			c.touchObject.swipeLength = k
		}
		h = c.swipeDirection();
	...
	b.prototype.unload = function() {
		...
		c.$slides.removeClass("slick-slide slick-active slick-visible slick-current").attr("aria-hidden",
			"true").css("width", "")
	};
	...
	b.prototype.updateDots = function() {
		...
		if (c.$dots !== null) {
			...
			c.$dots.find("li").eq(Math.floor(c.currentSlide / c.options.slidesToScroll)).addClass(
				"slick-active")
		}
	};

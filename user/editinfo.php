	...
	<div class="tab-content">
		<div class="tab-pane ng-scope active">
			<form class="form-horizontal devform">
				...
				<div class="form-group">
					<label class="col-sm-2 control-label">设备版本version</label>
					<div class="col-sm-9">
						<input class="form-control" type="number" name="voice_version" value="<?php echo $userrow['voice_version']?>">
					</div>
				</div>
				<div class="form-group">
...
$(document).ready(function(){
	...
	$("#editVoice").click(function(){
		...
		var voice_version=$("input[name='voice_version']").val();
		var voice_order=$("select[name='voice_order']").val();
		var ii = layer.load(2, {shade:[0.1,'#fff']});
		$.ajax({
			type : "POST",
			url : "ajax2.php?act=edit_voice",
			data : {voice_devid:voice_devid,voice_version:voice_version, voice_order:voice_order},
			dataType : 'json',
			success : function(data) {
				layer.close(ii);
				if(data.code == 1){
